...
def render_chart(chart_type, selected_brand, selected_model, fig=None, ax=None, is_comparison=False):
    if ax is None:
        ax = plt.gca()

    if chart_type == 'Heatmap':
        pivot = df.pivot_table(index='Brand', columns='Year', values='Units Sold (Millions)', aggfunc='sum', fill_value=0)
        ...
        if selected_model and selected_model != 'None' and selected_brand != 'All':
            ...
            if not filtered_df.empty:
                ...
                if chart_type == 'Bar Chart':
                    ...
                    ax.set_title(f"{selected_model} Sales Over the Years")
...
if not compare_button:
    ...
    render_chart(selected_chart, selected_brand, selected_model, fig, ax)
    st.pyplot(fig)
    ...
    if len(compare_charts) < 2:
        ...
        for idx, chart_type in enumerate(compare_charts):
            render_chart(chart_type, selected_brand, selected_model, fig, axes[idx], is_comparison=True)
...
if show_stats:
    if selected_brand == 'All':
        ...
        filtered_df = df[df['Brand'] == selected_brand] if selected_model == 'None' else \
                      df[(df['Brand'] == selected_brand) & (df['Model'] == selected_model)]
        model_text = 'All Models' if selected_model == 'None' else selected_model
        brand_data = filtered_df['Units Sold (Millions)']
        stats_text = f"""
        📊 **{selected_brand} ({model_text}) Statistics**:
